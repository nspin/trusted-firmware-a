/*
 * Copyright (c) 2013-2019, ARM Limited and Contributors. All rights reserved.
 *
 * SPDX-License-Identifier: BSD-3-Clause
 */

#ifndef LINUX_H
#define LINUX_H

#define LINUX_PUTCHAR		0xf200000a

/*
 * SMC function IDs that LINUX uses to signal various forms of completions
 * to the secure payload dispatcher.
 */
#define LINUX_ENTRY_DONE	0xf2000000
#define LINUX_ON_DONE		0xf2000001
#define LINUX_OFF_DONE		0xf2000002
#define LINUX_SUSPEND_DONE	0xf2000003
#define LINUX_RESUME_DONE	0xf2000004
#define LINUX_PREEMPTED		0xf2000005
#define LINUX_ABORT_DONE	0xf2000007
#define LINUX_SYSTEM_OFF_DONE	0xf2000008
#define LINUX_SYSTEM_RESET_DONE	0xf2000009

/*
 * Function identifiers to handle S-EL1 interrupt through the synchronous
 * handling model. If the LINUX was previously interrupted then control has to
 * be returned to the LINUXD after handling the interrupt else execution can
 * remain in the LINUX.
 */
#define LINUX_HANDLED_S_EL1_INTR		0xf2000006

/* SMC function ID that LINUX uses to request service from secure monitor */
#define LINUX_GET_ARGS		0xf2001000

/*
 * Identifiers for various LINUX services. Corresponding function IDs (whether
 * fast or yielding) are generated by macros defined below
 */
#define LINUX_ADD		0x2000
#define LINUX_SUB		0x2001
#define LINUX_MUL		0x2002
#define LINUX_DIV		0x2003
#define LINUX_HANDLE_SEL1_INTR_AND_RETURN	0x2004

/*
 * Identify a LINUX service from function ID filtering the last 16 bits from the
 * SMC function ID
 */
#define LINUX_BARE_FID(fid)	((fid) & 0xffff)

/*
 * Generate function IDs for LINUX services to be used in SMC calls, by
 * appropriately setting bit 31 to differentiate yielding and fast SMC calls
 */
#define LINUX_YIELD_FID(fid)	((LINUX_BARE_FID(fid) | 0x72000000))
#define LINUX_FAST_FID(fid)	((LINUX_BARE_FID(fid) | 0x72000000) | (1u << 31))

/* SMC function ID to request a previously preempted yielding smc */
#define LINUX_FID_RESUME		LINUX_YIELD_FID(0x3000)
/*
 * SMC function ID to request abortion of a previously preempted yielding SMC. A
 * fast SMC is used so that the LINUX abort handler does not have to be
 * reentrant.
 */
#define LINUX_FID_ABORT		LINUX_FAST_FID(0x3001)

/*
 * Total number of function IDs implemented for services offered to NS clients.
 * The function IDs are defined above
 */
#define LINUX_NUM_FID		0x5

/* LINUX implementation version numbers */
#define LINUX_VERSION_MAJOR	0x0 /* Major version */
#define LINUX_VERSION_MINOR	0x1 /* Minor version */

/*
 * Standard Trusted OS Function IDs that fall under Trusted OS call range
 * according to SMC calling convention
 */
#define TOS_CALL_COUNT		0xbf00ff00 /* Number of calls implemented */
#define TOS_UID			0xbf00ff01 /* Implementation UID */
/*				0xbf00ff02 is reserved */
#define TOS_CALL_VERSION	0xbf00ff03 /* Trusted OS Call Version */


#ifndef __ASSEMBLER__

#include <stdint.h>


typedef uint32_t linux_vector_isn_t;

typedef struct linux_vectors {
	linux_vector_isn_t yield_smc_entry;
	linux_vector_isn_t fast_smc_entry;
	linux_vector_isn_t cpu_on_entry;
	linux_vector_isn_t cpu_off_entry;
	linux_vector_isn_t cpu_resume_entry;
	linux_vector_isn_t cpu_suspend_entry;
	linux_vector_isn_t sel1_intr_entry;
	linux_vector_isn_t system_off_entry;
	linux_vector_isn_t system_reset_entry;
	linux_vector_isn_t abort_yield_smc_entry;
} linux_vectors_t;

void linux_setup(void);

#endif /* __ASSEMBLER__ */

#endif /* LINUX_H */
